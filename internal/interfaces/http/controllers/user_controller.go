...
func (h *UserController) EditUserPassword(c *fiber.Ctx) error {
	...
	if err := helper.ValidateStruct(userInput); err != nil {
		for _, err := range err.(validator.ValidationErrors) {
			...
			case "Id":
				return helper.RespondError(c, fiber.StatusBadRequest, "Invalid ID")
			case "OldPassword":
				return helper.RespondError(c, fiber.StatusBadRequest, "Invalid old password")
			case "Password":
				return helper.RespondError(c, fiber.StatusBadRequest, err.Error())
			default:
				return helper.RespondError(c, fiber.StatusBadRequest, err.Error())
	...
	return helper.RespondMessage(c, fiber.StatusOK, "success change password")
}
...
func (h *UserController) EditUser(c *fiber.Ctx) error {
	...
	return helper.RespondMessage(c, fiber.StatusOK, "success update user")
}
