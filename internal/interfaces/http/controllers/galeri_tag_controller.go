...
type GaleriTagController struct {
	galeriTagService service.GaleriTagService
}

func NewGaleriTagController(galeriTagService service.GaleriTagService) *GaleriTagController {
	return &GaleriTagController{galeriTagService: galeriTagService}
}

func (h *GaleriTagController) GetAllGaleriTags(c *fiber.Ctx) error {
	...
	blogs, err := h.galeriTagService.FindAllWithPagination(c.Context(), per_page, offset)
	if err != nil {
...
func (h *GaleriTagController) GetGaleriTagById(c *fiber.Ctx) error {
	id, err := strconv.Atoi(c.Params("id"))
	if err != nil {
		return helper.RespondError(c, fiber.StatusBadRequest, "Invalid ID")
	}

	blog, err := h.galeriTagService.FindById(c.Context(), id)
	if err != nil {
		if e, ok := err.(helper.AppError); ok {
			return helper.RespondError(c, e.Code, e.Message)
		}
...
func (h *GaleriTagController) CreateGaleriTag(c *fiber.Ctx) error {
	userInput := new(dto.GaleriTagCreate)
	...
	if _, err := h.galeriTagService.Create(c.Context(), userInput); err != nil {
		if e, ok := err.(helper.AppError); ok {
			return helper.RespondError(c, e.Code, e.Message)
		}
		return helper.RespondError(c, fiber.StatusInternalServerError, err.Error())
...
func (h *GaleriTagController) EditGaleriTag(c *fiber.Ctx) error {
	id, err := strconv.Atoi(c.Params("id"))
	if err != nil {
		return helper.RespondError(c, fiber.StatusBadRequest, "Invalid ID")
	}

	userInput := new(dto.GaleriTagUpdate)
	if err = c.BodyParser(userInput); err != nil {
		return helper.RespondError(c, fiber.StatusBadRequest, err.Error())
	}
	...
	if err = h.galeriTagService.Update(c.Context(), userInput); err != nil {
		if e, ok := err.(helper.AppError); ok {
			return helper.RespondError(c, e.Code, e.Message)
		}
		return helper.RespondError(c, fiber.StatusInternalServerError, err.Error())
...
func (h *GaleriTagController) DeleteGaleriTag(c *fiber.Ctx) error {
	id, err := strconv.Atoi(c.Params("id"))
	if err != nil {
		return helper.RespondError(c, fiber.StatusBadRequest, "Invalid ID")
	}

	if err = h.galeriTagService.Delete(c.Context(), id); err != nil {
		if e, ok := err.(helper.AppError); ok {
			return helper.RespondError(c, e.Code, e.Message)
		}
		return helper.RespondError(c, fiber.StatusInternalServerError, err.Error())
